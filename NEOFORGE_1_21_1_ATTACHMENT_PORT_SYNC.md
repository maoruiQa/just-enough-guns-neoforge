...
- Draw/reload interaction parity is partially wired:
  ...
  - The client consumes the queued reload-cancel draw before the normal `isReloading(stack)` guard, strips the stale `gun_reload_ticks_*` visual components from the render stack, and force-resets the controller without clearing GeckoLib's live snapshot cache. This prevents the interrupted gun from staying in the down-pressed reload pose for several seconds after switching back without triggering GeckoLib 4.8.3's null snapshot crash during first-person item rendering.
