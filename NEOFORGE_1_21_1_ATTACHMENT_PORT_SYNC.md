...
- Draw/reload interaction parity is partially wired:
  ...
  - The client consumes the queued reload-cancel draw before the normal `isReloading(stack)` guard, strips the stale `gun_reload_ticks_*` visual components from the render stack, and clears the GeckoLib snapshot cache when forcing the controller reset. This prevents the interrupted gun from staying in the down-pressed reload pose for several seconds after switching back.
