...
- Draw/reload interaction parity is wired:
  ...
  - Client draw restarts reset and stop the controller, then return the normal predicate `DRAW` animation instead of using GeckoLib's triggerable `draw` layer. This keeps switch-back draw visible without leaving a triggered `draw` animation that can suppress first-person sprint animation.
  - When first-person sprinting interrupts draw, the client clears pending/recent draw state and records a short sprint-suppressed draw window so stopping sprint does not replay the interrupted switch animation.
