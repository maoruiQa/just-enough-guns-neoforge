...
public final class DamageModifierInfo {
    ...
    public record Rule(Op op, float value, MatchKind kind, String matchId) {
        ...
        private static boolean matchesEntityId(DamageSource source, String id) {
            ...
            if (direct != null) {
                Identifier typeId = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.getKey(direct.getType());
                if (typeId != null && (typeId.toString().equals(key) || typeId.getPath().equals(key))) {
                    return true;
                }
            }
            if (causing != null) {
                Identifier typeId = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.getKey(causing.getType());
                if (typeId != null && (typeId.toString().equals(key) || typeId.getPath().equals(key))) {
                    return true;
                }
            }
