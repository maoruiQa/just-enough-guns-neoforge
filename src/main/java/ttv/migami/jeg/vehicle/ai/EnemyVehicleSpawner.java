...
public final class EnemyVehicleSpawner {
    ...
    private static VehicleEntity spawnVehicle(ServerLevel level, BlockPos pos, float yaw, Identifier vehicleId, @Nullable PathfinderMob existingCrew, Vec3 anchor) {
        EntityType<? extends VehicleEntity> type = vehicleType(vehicleId);
        VehicleEntity vehicle = EnemyVehicleSpawnItem.spawnVehicle(level, pos, type, vehicleId, null, null, false, existingCrew);
        if (vehicle != null) {
            vehicle.setYRot(yaw);
    ...
    private static boolean isJungle(ServerLevel level, BlockPos pos) {
        ...
        return biome.unwrapKey().map(key -> key.toString().contains("jungle")).orElse(false);
    }
