...
public final class DefaultVehicleResource {
    ...
    private static final Identifier FALLBACK_MODEL = Reference.id(MODEL_ROOT + FALLBACK + ".geo.json");
    private static final Identifier FALLBACK_ANIMATION = Reference.id(ANIMATION_ROOT + FALLBACK + ".animation.json");
    ...
    public static Identifier model(VehicleEntity vehicle) {
        ...
        Identifier lodModel = lodResource(vehicle, MODEL_LOD_ROOT, ".geo.json");
        if (lodModel != null) {
            return lodModel;
        }
        Identifier model = Reference.id(MODEL_ROOT + vehiclePath + ".geo.json");
        if (exists(model)) {
            return model;
        }
        Identifier typeModel = Reference.id(MODEL_ROOT + typeFallback(vehicle) + ".geo.json");
    ...
    public static Identifier animation(VehicleEntity vehicle) {
        Identifier animation = Reference.id(ANIMATION_ROOT + vehiclePath(vehicle) + ".animation.json");
        if (exists(animation)) {
            return animation;
        }
        Identifier typeAnimation = Reference.id(ANIMATION_ROOT + typeFallback(vehicle) + ".animation.json");
