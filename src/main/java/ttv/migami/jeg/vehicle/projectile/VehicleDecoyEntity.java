...
/**
 * Flare / smoke decoy. Smoke display matches Superb Warfare SmokeDecoyEntity exactly:
 * optional one-shot ignite burst only — no continuous emission.
 */
public final class VehicleDecoyEntity extends Entity {
    ...
    /** SW releaseSmoke: vehicle true (burst), grenade-spawned false (silent blocker). */
    ...
    public EntityDimensions getDimensions(Pose pose) {
        // SW SmokeDecoyEntity sized(4.5, 4.5)
    ...
    /** Heat-seeking missiles should only chase flares, never smoke screens. */
    ...
    private void tickSmoke() {
        // SW SmokeDecoyEntity.tick
        this.move(MoverType.SELF, this.getDeltaMovement());
        this.reapplySmokeBounds();

        if (this.tickCount == SMOKE_IGNITE_TICKS) {
            if (this.releaseSmoke && this.level() instanceof ServerLevel serverLevel) {
                // SW: 50 CustomSmoke at xo/yo/zo, count 50, offsets 0, speed 0.07, force
                serverLevel.sendParticles(WHITE_SMOKE, true, false, this.xo, this.yo, this.zo, 50, 0.0D, 0.0D, 0.0D, 0.07D);
                // SW: LARGE_SMOKE 10, offsets 1,1,1, speed 0.1
                serverLevel.sendParticles(ParticleTypes.LARGE_SMOKE, true, false, this.xo, this.yo, this.zo, 10, 1.0D, 1.0D, 1.0D, 0.1D);
                // SW: FIRE_STAR 30, speed 0.2 — FLAME is the closest stock substitute
                serverLevel.sendParticles(ParticleTypes.FLAME, true, false, this.xo, this.yo, this.zo, 30, 0.0D, 0.0D, 0.0D, 0.2D);
        ...
        // Report lock volume every tick on both sides (SW has no continuous particles for decoys).
    ...
    protected void readAdditionalSaveData(ValueInput input) {
        this.entityData.set(DATA_SMOKE, input.getBooleanOr("Smoke", false));
        this.releaseSmoke = input.getBooleanOr("ReleaseSmoke", true);
        if (this.isSmokeDecoy()) {
            this.refreshDimensions();
            this.reapplySmokeBounds();
        }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        output.putBoolean("Smoke", this.isSmokeDecoy());
        output.putBoolean("ReleaseSmoke", this.releaseSmoke);
    }
