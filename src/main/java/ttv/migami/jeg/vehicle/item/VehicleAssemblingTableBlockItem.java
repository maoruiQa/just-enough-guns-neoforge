    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            ...
            public GeoItemRenderer<?> getGeoItemRenderer() {
                if (renderer == null) {
                    try {
                        Class<?> clazz = Class.forName("ttv.migami.jeg.vehicle.client.render.item.VehicleAssemblingTableBlockItemRenderer");
                        renderer = (GeoItemRenderer<?>) clazz.getDeclaredConstructor().newInstance();
                    } catch (Exception e) {
                        throw new RuntimeException("Failed to create VehicleAssemblingTableBlockItemRenderer", e);
                    }
                }
                return renderer;
            }
