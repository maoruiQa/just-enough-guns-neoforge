...
public final class KillEffectEffect extends IncurableEffect {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (entity.hasEffect(ModEffects.POPPED)) {
            level.sendParticles(
            ...
            if (level.random.nextBoolean()) {
                level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.BEEHIVE_EXIT, SoundSource.PLAYERS, 5.0F, 0.5F + level.random.nextFloat());
            }
        }
        if (entity.hasEffect(ModEffects.TRICKSHOTTED)) {
            level.sendParticles(
