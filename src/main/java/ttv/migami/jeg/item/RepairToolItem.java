...
public class RepairToolItem extends Item implements GeoItem {
    ...
    public RepairToolItem(Properties properties) {
        super(properties.rarity(Rarity.COMMON).stacksTo(1).durability(MAX_USES));
    ...
    private void playSound(ServerLevel level, Vec3 pos) {
        ...
        level.playSound(null, pos.x, pos.y, pos.z, sound, SoundSource.PLAYERS, 0.7F, 0.95F + level.getRandom().nextFloat() * 0.1F);
    }
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, TooltipDisplay display, Consumer<Component> tooltipAdder, TooltipFlag flag) {
        tooltipAdder.accept(Component.translatable("info.jeg.repair_tool.vehicle").withStyle(ChatFormatting.AQUA));
        tooltipAdder.accept(Component.translatable("info.jeg.repair_tool.shift").withStyle(ChatFormatting.GRAY));
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new com.geckolib.animation.AnimationController<>(
                CONTROLLER,
                0,
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            ...
            public GeoItemRenderer<?> getGeoItemRenderer() {
                if (renderer == null) {
                    try {
                        Class<?> clazz = Class.forName("ttv.migami.jeg.client.render.item.RepairToolItemRenderer");
                        renderer = (GeoItemRenderer<?>) clazz.getDeclaredConstructor().newInstance();
                    } catch (Exception e) {
                        throw new RuntimeException("Failed to create RepairToolItemRenderer", e);
                    }
                }
                return renderer;
            }
