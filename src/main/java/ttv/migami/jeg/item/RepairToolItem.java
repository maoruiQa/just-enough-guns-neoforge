...
public class RepairToolItem extends Item implements GeoItem {
    ...
    public void createGeoRenderer(Consumer consumer) {
        try {
                    ...
                    (proxy, method, args) -> {
                        if ("getGeoItemRenderer".equals(method.getName())) {
                            if (cachedGeoItemRenderer == null) {
                                Class<?> rendererClass = Class.forName("ttv.migami.jeg.client.render.item.RepairToolItemRenderer");
            ...
            throw new RuntimeException("Failed to create RepairToolItemRenderer provider", e);
        }
    }
}
