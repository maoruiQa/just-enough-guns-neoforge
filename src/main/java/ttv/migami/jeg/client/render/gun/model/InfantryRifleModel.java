...
public class InfantryRifleModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        ...
        RenderUtil.renderModel(SpecialModels.INFANTRY_RIFLE_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if ((Gun.hasAttachmentEquipped(stack, IAttachment.Type.MAGAZINE)))
        {
            if (Gun.getAttachment(IAttachment.Type.MAGAZINE, stack).getItem() == ModItems.EXTENDED_MAG.get())
                RenderUtil.renderModel(SpecialModels.INFANTRY_RIFLE_MAGAZINE_EXTENDED.getModel(), stack, matrixStack, buffer, light, overlay);
            if (Gun.getAttachment(IAttachment.Type.MAGAZINE, stack).getItem() == ModItems.DRUM_MAG.get())
                RenderUtil.renderModel(SpecialModels.INFANTRY_RIFLE_MAGAZINE_DRUM.getModel(), stack, matrixStack, buffer, light, overlay);
        }
        else
            RenderUtil.renderModel(SpecialModels.INFANTRY_RIFLE_MAGAZINE_DEFAULT.getModel(), stack, matrixStack, buffer, light, overlay);
