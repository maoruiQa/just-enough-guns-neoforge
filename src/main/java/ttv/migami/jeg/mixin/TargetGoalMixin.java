...
public abstract class TargetGoalMixin {
    ...
    private boolean jeg$hasLineOfSight(Sensing sensing, net.minecraft.world.entity.Entity target) {
        if (this.mob.hasEffect(ModEffects.SMOKED) || SmokeUtil.isInSmoke(this.mob)) {
            return false;
        }
        if (target instanceof LivingEntity living && (living.hasEffect(ModEffects.SMOKED) || SmokeUtil.isInSmoke(living))) {
            return false;
        }
        if (this.mob.entityTags().contains("MobGunner") && target instanceof Player player && player.isInvisible()) {
            return false;
        }
