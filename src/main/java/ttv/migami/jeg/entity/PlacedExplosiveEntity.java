...
public final class PlacedExplosiveEntity extends Entity implements GeoEntity {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ATTACHED = SynchedEntityData.defineId(PlacedExplosiveEntity.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Optional<UUID>> OWNER = SynchedEntityData.defineId(PlacedExplosiveEntity.class, EntityDataSerializers.OPTIONAL_UUID);
    ...
    public PlacedExplosiveEntity(ServerLevel level, SpecialExplosiveItem.Kind kind, @Nullable LivingEntity owner, Vec3 position, float yaw) {
        ...
        this.entityData.set(OWNER, Optional.ofNullable(owner == null ? null : owner.getUUID()));
    ...
    public static PlacedExplosiveEntity throwFrom(ServerLevel level, LivingEntity owner, SpecialExplosiveItem.Kind kind, boolean remoteOrFuse) {
        ...
        if (kind == SpecialExplosiveItem.Kind.C4) {
            ...
            float randomRot = (float) Mth.clamp((2.0D * level.random.nextDouble() - 1.0D) * 180.0D, -180.0D, 180.0D);
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(ATTACHED, Optional.empty());
    ...
    public boolean isInGround() {
        return this.entityData.get(IN_GROUND) || this.settled || this.entityData.get(ATTACHED).isPresent();
    }
    ...
    private void tickC4() {
        Optional<UUID> attachedId = this.entityData.get(ATTACHED);
        if (attachedId.isPresent()) {
    ...
    private void tickProjectileMotion(boolean sticky) {
        if (this.entityData.get(IN_GROUND) || this.entityData.get(ATTACHED).isPresent()) {
            this.setDeltaMovement(Vec3.ZERO);
            return;
        }
    ...
    public void detonate(boolean weak) {
        ...
        Optional<UUID> attached = this.entityData.get(ATTACHED);
        if (attached.isPresent()) {
            Entity target = serverLevel.getEntity(attached.get());
            if (target != null) {
                pos = target.position();
            }
        }
    ...
    private void playSound(net.minecraft.resources.ResourceLocation id, float volume, float pitch) {
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (this.level().isClientSide || amount <= 0.0F || this.isRemoved() || this.detonating) {
            return false;
        }
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.entityData.set(BOMB_TICK, tag.getInt("BombTick"));
        this.entityData.set(HEALTH, tag.getFloat("Health"));
        this.entityData.set(IN_GROUND, tag.getBoolean("InGround"));
    ...
    protected void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putInt("BombTick", this.entityData.get(BOMB_TICK));
        tag.putFloat("Health", this.entityData.get(HEALTH));
        tag.putBoolean("InGround", this.entityData.get(IN_GROUND));
