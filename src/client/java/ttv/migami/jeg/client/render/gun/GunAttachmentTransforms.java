...
public final class GunAttachmentTransforms {
    ...
    private static final Map<Identifier, Map<AttachmentType, Transform>> TRANSFORMS = Map.ofEntries(
            ...
            entry("custom_smg", barrel(0.0D, 4.475D, -1.2D), stock(0.0D, 3.0D, 8.1D), special(-0.8D, 3.8D, 0.0D)),
            ...
            entry("phantom_smg", barrel(0.0D, 4.475D, -1.2D), stock(0.0D, 3.0D, 8.1D), special(-0.8D, 3.8D, 0.0D)),
    ...
    public static Optional<Transform> transform(Identifier gunId, AttachmentType type) {
        return Optional.ofNullable(TRANSFORMS.get(gunId)).map(slots -> slots.get(type));
    }

    private static Map.Entry<Identifier, Map<AttachmentType, Transform>> entry(String gunId, SlotTransform... transforms) {
