...
/**
 * Renderer for PhantomGunner using the legacy rendering system (1.21.1).
...
public final class PhantomGunnerRenderer extends MobRenderer<PhantomGunner, PhantomModel<PhantomGunner>> {
    ...
    protected void scale(PhantomGunner entity, PoseStack poseStack, float partialTickTime) {
        ...
        float scale = 1.0F + 0.2F * size;
        poseStack.scale(scale, scale, scale);
        poseStack.translate(0.0F, 1.3125F, 0.1875F);
    }

    @Override
    protected void setupRotations(PhantomGunner entity, PoseStack poseStack, float bob, float yBodyRot, float partialTick, float scale) {
