...
public final class ClientHudRenderer {
    ...
    public static void render(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawString(minecraft.font, fit(minecraft, data.name(), textWidth), startX, startY - 10, data.nameColor(), true);

        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(startX, startY, 0.0F);
        guiGraphics.pose().scale(2.0F, 2.0F, 1.0F);
        guiGraphics.drawString(minecraft.font, formatCurrent(data.currentAmmo()), 0, 0, 0xFFFFFF, true);
        guiGraphics.pose().popPose();

        guiGraphics.drawString(minecraft.font, data.reserveText(), startX + 38, startY, 0x878787, true);
    }
    ...
    private static HudData buildGunHud(LocalPlayer player, ItemStack stack, GunItem gun) {
        ...
        return new HudData(stack.getHoverName().getString(), current, reserveText, rarityColor(stack));
    }
