...
public final class CrosshairHandler {
    ...
    private static final Identifier DYNAMIC_H = Reference.id("textures/crosshair/dynamic_horizontal.png");
    private static final Identifier DYNAMIC_V = Reference.id("textures/crosshair/dynamic_vertical.png");
    ...
    private static void renderTech(GuiGraphicsExtractor guiGraphics, int width, int height, float partialTick) {
        ...
        pose.rotate((float) Math.toRadians(Mth.lerp(partialTick, prevTechRotation, techRotation)));
    ...
    private static void renderDynamic(GuiGraphicsExtractor guiGraphics, LocalPlayer player, GunItem gun, int width, int height, float partialTick) {
        ...
        ItemStack stack = player.getMainHandItem();
        float spread = GunItem.getClientSpreadDegrees(player, stack, gun.getStats(), aiming > 0.5F);
    ...
    private static void drawDynamicPart(GuiGraphicsExtractor guiGraphics, Identifier texture, float x, float y, float width, float height,
                                        int sourceX, int sourceY, int sourceWidth, int sourceHeight) {
        var pose = guiGraphics.pose();
        pose.pushMatrix();
        pose.translate(x, y);
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture, Math.round(-width / 2.0F), Math.round(-height / 2.0F),
                sourceX, sourceY, Math.round(width), Math.round(height), sourceWidth, sourceHeight, DYNAMIC_TEXTURE_SIZE, DYNAMIC_TEXTURE_SIZE);
        pose.popMatrix();
    }
    ...
    private static void blitCentered(GuiGraphicsExtractor guiGraphics, Identifier texture, float size, float alpha) {
        int intSize = Math.round(size);
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture, -intSize / 2, -intSize / 2, 0.0F, 0.0F,
                intSize, intSize, CROSSHAIR_TEXTURE_SIZE, CROSSHAIR_TEXTURE_SIZE, CROSSHAIR_TEXTURE_SIZE, CROSSHAIR_TEXTURE_SIZE, alphaColor(alpha));
    }
